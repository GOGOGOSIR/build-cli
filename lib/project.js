...
class Project {
  ...
  buildMain(answers) {
    ...
    try {
      const { projectName, distName, cwd } = this.cfg
      ...
      const { stdout: currentBranch } = execaSync('git', ['branch', '--show-current'], { stdio: 'pipe' })
      ...
      execaSync('zip', ['-r', `${projectName}.zip`, distName || projectName], { stdio: 'inherit' })
